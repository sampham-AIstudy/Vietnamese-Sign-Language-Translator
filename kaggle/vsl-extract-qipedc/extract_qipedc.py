"""
Kaggle job: extract 67-joint landmarks for every QIPEDC video with the backend's extractor.
Runs under an isolated Python 3.11 + mediapipe 0.10.14 env (same version as the backend).
Output: /kaggle/working/qipedc_kps/*.npz + manifest.csv
"""
...
sh(f"cd /tmp/vslt && /tmp/mp311/bin/python scripts/extract_keypoints_batch.py "
   f"--videos-dir '{videos_dir}' --labels-csv '{labels[0]}' --source qipedc "
   f"--out-dir /kaggle/working/qipedc_kps --workers {os.cpu_count()}")

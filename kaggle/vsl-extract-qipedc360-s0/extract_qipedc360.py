"""
Kaggle job: extract 67-joint landmarks for QIPEDC shard 0/3 at 360 px height with the backend's extractor.
...
sh(f"cd /tmp/vslt && /tmp/mp311/bin/python scripts/extract_keypoints_batch.py "
   f"--videos-dir '{videos_dir}' --labels-csv '{labels[0]}' --source qipedc --process-height 360 --shard 0/3 "

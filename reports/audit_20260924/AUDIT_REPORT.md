...
| **B4** | Dữ liệu Cấp 3 (CSLR & Parallel) | **PASS** | CSLR: 4.200 file keypoints `.npy` (VSL-GH, 300 câu y tế thật, 6 signers). Parallel text: 10K ~~là dữ liệu tổng hợp (PCFG rule-based)~~ **(đính chính vòng 3: không có căn cứ cho "PCFG"; upstream không công bố phương pháp biên soạn → UNVERIFIED; 9.404/9.405 cặp khớp upstream — `reports/audit_round3/PROVENANCE.md`)** (9.405 câu thô, 7.140 câu đã làm sạch). |
...
| **E1** | Tái lập chỉ số Cấp 2 (Held-out Test) | **PASS** | Chạy lại trên 487 mẫu `tier2_indomain_test.csv`: **Top-1 = 46.41%**, **Top-5 = 60.78%** (Khớp chính xác 100% với số liệu đã công bố). **Đính chính 24/09 (vòng 3):** tái lập đúng nhưng split rò rỉ bản quay trùng; clip sạch Top-1 = 8.07% — xem `reports/audit_round3/PROVENANCE.md`. |
| **E2** | Rà soát phân bổ phương ngữ Cấp 2 | **PASS** | Miền Trung đạt 61.73%, Miền Nam đạt 56.17%, Miền Bắc chỉ đạt 21.47% ~~(do tập train indomain chứa nhiều dữ liệu Trung/Nam hơn)~~ **(đính chính: do tỉ lệ bản quay trùng với train — Trung 80%, Nam 59%, Bắc 23%; xem audit vòng 3)**. |
...
| **Dịch thuật (Gloss $\to$ Text)** | **ĐỦ CHO MIỀN Y TẾ**<br>(10K cleaned — phương pháp biên soạn UNVERIFIED + 300 câu VSL-GH) | Thiếu câu song song đã xác minh do người Điếc dịch (phương pháp biên soạn ngữ liệu 10K chưa rõ — không có căn cứ cho "sinh bằng luật"). | Khai thác thêm dữ liệu văn bản từ sách dạy ngôn ngữ ký hiệu hoặc từ điển mở. | **P1** | Tăng độ tự nhiên và ngữ pháp tiếng Việt khi dịch các câu phức tạp. |
